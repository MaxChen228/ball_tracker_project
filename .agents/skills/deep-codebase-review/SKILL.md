...
pwd
git fetch origin
git checkout -b Codex/audit-fix-[DOMAIN] origin/main  # base on BASE_HASH
...
## 完成
git add -A && git commit -m "..." && git push -u origin Codex/audit-fix-[DOMAIN]
最後 message 以 `Branch: Codex/audit-fix-[DOMAIN]` 結尾。
```
...
```bash
git fetch origin
git ls-remote origin Codex/audit-fix-[DOMAIN]  # branch 真的在 origin？
git log --oneline origin/main..origin/Codex/audit-fix-[DOMAIN]  # 真的有 worker 的 commit？
git diff origin/main..origin/Codex/audit-fix-[DOMAIN] --stat | head -10  # 改動範圍含預期檔案？
```
...
```
你是 [project] 的 deep reviewer，審 `Codex/audit-fix-[DOMAIN]`。不寫程式 / 不 commit。
...
## Setup
git fetch origin
git checkout origin/Codex/audit-fix-[DOMAIN]
git diff origin/main..HEAD --stat
...
```
git checkout -b Codex/audit-batch-<date> origin/main
# 按 cross-cutting reviewer 推薦順序 cumulative merge
git merge --no-ff -m "merge: [domain]" origin/Codex/audit-fix-[domain]
# ... 每條一次
cd server && uv run pytest -x
git push -u origin Codex/audit-batch-<date>
```
