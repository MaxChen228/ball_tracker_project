...
- `routes/device_ws.py` — PR #85 phase 2 abstraction: contains the `ws_device` handler that processes `frame` / `heartbeat` / `hello` / `cycle_end` / `sync_report` / `calibration_frame` WS messages on `/ws/device/{cam}`. Pulled out of `main.py` so the FastAPI app file stays the wiring root.
