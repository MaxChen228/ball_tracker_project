...
**iOS-decoupling status (Phase 1-5 complete)**: the dashboard at `/` is the sole control surface. Per-session operator flow requires zero iPhone touches: enable preview per cam → **Auto calibrate** (server-side ArUco) → **Calibrate time** (single-listener chirp) or `/sync` page (mutual two-device chirp) → **Arm session** → throw ball → event auto-captured. iOS UI is display-only status (`AutoCalibrationViewController` kept as field fallback; no local time-sync button; no Manual 5-handle path). Per-camera intrinsics + homography live in `data/calibrations/<cam>.json` (authoritative — iOS no longer echoes on pitch uploads). HSV / chirp threshold / heartbeat interval / extended markers all server-persisted + pushed via heartbeat.
