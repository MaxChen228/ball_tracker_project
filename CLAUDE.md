...
Runs on `camera.frame.queue` at **60 fps while idle (`.standby` / `.timeSyncWaiting`) and 240 fps while tracking (`.syncWaiting` / `.recording` / `.uploading`)**. The switch is driven by `switchCaptureFps(_:)` called from `enterSyncMode` / `exitSyncMode`; each transition costs a `stopRunning → activeFormat = X → startRunning` cycle (~300-500 ms), deliberately spent in the non-time-critical windows (right before the ball is expected, and after the session ends). Advances `frameIndex` every frame (cross-mode coherence). **Never** `DispatchQueue.main.async` per frame — detection results are stashed in `latestCentroidX/Y` and a `CADisplayLink` on main redraws the overlay at 60/120 Hz. Only actual state transitions dispatch to main.
...
- `CalibrationViewController.swift` — two paths to the same `homography_3x3` (row-major, 9 doubles, h33=1):
  ...
  Also derives `fx/fz/cx/cy` from capture FOV. A Settings toggle can override intrinsics with externally computed ChArUco values including 5-coefficient distortion. Both save paths (manual Save + Auto ArUco Save) also POST the freshly-persisted `{camera_id, intrinsics, homography, image_{width,height}_px}` to the server's `/calibration` endpoint via `ServerUploader.postCalibration` — fire-and-forget, failures log only. This feeds the dashboard's calibration-preview canvas so each phone's pose appears immediately after Save, without waiting for the first pitch.
- `SettingsViewController.swift` — persists server IP/port, role A/B, HSV range, chirp detection threshold (`chirp_threshold`, default 0.18), heartbeat interval (`poll_interval_s`, default 1, clamped [1, 60]), and optional manual intrinsics (including OpenCV 5-coefficient distortion `[k1, k2, p1, p2, k3]`) to `UserDefaults`. **Capture resolution is system-wide fixed at 1920×1080** (`captureWidthFixed` / `captureHeightFixed` in the file; the `captureWidth`/`captureHeight` struct fields and UserDefaults keys remain so `CameraViewController.selectFormat` stays parameterised, but any stale value is ignored on load). **Capture FPS is not a user setting** — it is owned by `CameraViewController` as `standbyFps = 60` / `trackingFps = 240` constants and switched automatically on entering/leaving `.syncWaiting`. `normalizeServerIP` strips scheme/port/path from pasted URLs. The Intrinsics section accepts `calibrate_intrinsics.py` JSON via `UIDocumentPickerViewController` and auto-crops/scales 4:3 → 1080p 16:9. Save validates port ∈ [1, 65535], heartbeat ∈ [1, 60], chirp ∈ (0, 1], HSV min<max, and fx/fy > 100 before persisting; otherwise shows an alert and refuses to dismiss. `onDismiss` fires in `viewDidDisappear` (covers Save, Cancel, and interactive-swipe dismiss — the last is blocked via `isModalInPresentation = true`) — the presenter re-diffs settings there instead of relying on a polling tick.

### Server
- `schemas.py` — Pydantic wire contracts (`IntrinsicsPayload`, `FramePayload`, `PitchPayload`, `CalibrationSnapshot`, `TriangulatedPoint`, `SessionResult`, `HeartbeatBody`) and the in-memory `Device` / `Session` dataclasses. `CalibrationSnapshot` is the body of `POST /calibration`: same `intrinsics + homography + image_{width,height}_px` shape `PitchPayload` already carries, so the server-side pose math (`_camera_view_from_intrinsics_and_homography` in `reconstruct.py`) reuses one codepath for both. `camera_id` is `^[A-Za-z0-9_-]{1,16}$` and `session_id` is `^s_[0-9a-f]{4,32}$` — both patterns are path-safe so file routes can interpolate them. `_DEFAULT_SESSION_TIMEOUT_S = 60.0` lives here too so tests can import it without pulling in FastAPI. `main.py` re-exports these names for back-compat — new callers should import from `schemas` directly.
...
- `reconstruct.py` — pure-geometry scene builder used by the viewer endpoints. `build_scene(session_id, pitches, triangulated)` returns a `Scene` dataclass with `cameras` (position + local RGB-triad axes in world frame), `rays` (one per ball-detected frame: origin = camera center, endpoint = ground-plane intersection or `_RAY_MAX_LEN_M` fallback), and `triangulated` points when both A+B are paired. `build_calibration_scene(calibrations)` is the companion used by the dashboard canvas — same `CameraView` shape but no rays / no triangulation, for previewing "where does each phone think it is" before any pitch exists. Both paths funnel through `_camera_view_from_intrinsics_and_homography` so the pose recovered from a `CalibrationSnapshot` is byte-for-byte the pose a later `PitchPayload` would produce. `Scene.to_dict()` is the stable JSON hand-off for `/reconstruction/{session_id}`.
...
- `render_scene.py` — `render_scene_html(scene)` / `render_scene_div(scene)` and the shared `_build_figure(scene)` underneath. Colour tokens (`_BG / _SURFACE / _INK / _CONTRA / _DUAL / _ACCENT / …`) mirror the dashboard's `PHYSICS_LAB` design-system palette so the viewer page and the dashboard canvas look like the same document. Cameras are tinted `contra` (A) / `dual` (B); the ground mesh is `borderL`; world axes are ink-muted so semantic colour stays on data, not decoration.
- `render_dashboard.py` — `render_events_index_html(events, devices, session, calibrations)` builds the three-zone dashboard (nav / 440 px sidebar / full-bleed 3D canvas). Owns `_CSS` (design-system tokens + card / chip / scrollbar rules) and `_JS_TEMPLATE` (three staggered poll ticks: `/status` at 1 s, `/calibration/state` at 5 s, `/events` at 5 s). Initial SSR hydrates every panel + the canvas via `fig.to_html(include_plotlyjs=False, full_html=False)`, so the first paint already shows device chips, the session chip, the events list, and the calibration scene; subsequent ticks call `Plotly.react` to swap in new trace/layout data. Plotly.js is loaded once at the top of the document from CDN.
- `main.py` — FastAPI app, routes, lifespan, and the `State` class (thread-locked). `State.pitches` dict keyed by `(camera_id, session_id)`; when both A and B for a session arrive, `triangulate_cycle` runs immediately under the state lock and the result is cached in `State.results[session_id]` as a `SessionResult`. `State._calibrations` (per-camera) is written by `POST /calibration` and persisted atomically to `data/calibrations/{camera_id}.json` — reloaded on startup so the dashboard survives a server restart. `/pitch` accepts multipart (`payload: str` Form + optional `video: UploadFile`); clip bytes are written atomically via tmp-rename to `data/videos/session_{session_id}_{camera_id}.{ext}`. `State.events()` collapses each session into one summary row for the `/events` JSON endpoint and the dashboard. A pitch arriving during an armed session auto-ends the session (one-shot arm) and flips commands to `disarm` for the echo window. Also hosts the `/sessions/arm`, `/sessions/cancel`, `/heartbeat`, `/chirp.wav`, `/status`, `/reset`, `/calibration`, `/calibration/state`, and `/reconstruction/{session_id}` routes.
