...
- **iOS app** — `ball_tracker/` (Swift + Obj-C++, UIKit). Xcode project at `ball_tracker.xcodeproj`. Bundle ID `com.Max0228.ball-tracker`, iOS 26.2 target, Swift 5.0. `LiveFrameDispatcher` streams per-frame detections over WS and `ClipRecorder` writes the MOV — both run unconditionally on every armed session. `currentSessionPaths` lingers in code as a vestige of the retired path-selection UI but doesn't gate behavior anymore.
...
- `pipeline.py` — glue. `detect_pitch(mov_path, video_start_pts_s, hsv_range?, frame_iter?, *, enable_bg_subtraction?, shape_gate?, selector_tuning?, chain_filter_params?, expected_radius_px?, should_cancel?)` decodes the MOV and synthesises one `FramePayload` per sample (pixel-path; `theta_*` stays None). When `enable_bg_subtraction=True` (default, dashboard-toggleable via `POST /detection/bg_subtraction`) it prepends an MOG2 subtractor + 3×3 `MORPH_CLOSE` on the foreground mask before AND-ing with HSV, then runs the same `detect_ball` selector as the live path; runs `chain_filter.annotate` once at the end. Tests inject a stub iterator to avoid real decoding.
