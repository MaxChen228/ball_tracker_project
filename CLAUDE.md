...
Every armed session snapshots a set of `DetectionPath`s onto `Session.paths` — any combination of `live`, `server_post` — so a dashboard flip mid-cycle can't disturb a running session. The legacy `CaptureMode` enum has been retired; `default_paths` / `session.paths` are the sole authority. `/status` still ships a degenerate `"capture_mode": "camera_only"` literal purely so older dashboard JS / iOS builds that read the key don't 500.
...
Detection constants are kept lock-step across both — see the header comment in `ball_tracker/BallDetector.mm` and the shared `server/detection.py` + `server/pipeline.py`. Viewer overlays each path independently (two pill sets, two A/B strip rows), so a path that found the ball where the other missed reads as a visible delta rather than a silent override.
...
- **`payload`** (`application/json`) — encoded `ServerUploader.PitchPayload` ↔ `main.PitchPayload`:
  ...
  paths: list[DetectionPath]                      # subset of {live, server_post}; snapshots session.paths at recording start
...
- **Live path has no HTTP payload** — `live` ∈ paths produces WS `frame` messages on `/ws/device/{cam}` throughout the recording. `state.persist_live_frames(camera_id, session_id)` flushes the in-memory buffer onto the pitch JSON at `path_completed` (or session end) so reloads see the same two-bucket shape as an offline upload.
