...
### Server
- `main.py` — `State.pitches` dict keyed by `(camera_id, session_id)`, plus an in-memory device registry and a single-slot session machine (both cleared on restart). When both A and B for a session arrive, triangulation runs immediately under the state lock and the result is stored in `State.results[session_id]` as a `SessionResult`. `/pitch` accepts multipart (`payload: str` Form + optional `video: UploadFile`); `camera_id` and `session_id` are both Pydantic-constrained (`^[A-Za-z0-9_-]{1,16}$` / `^s_[0-9a-f]{4,32}$`) so they can't escape the data dir when interpolated into file paths. Clip bytes are written atomically via tmp-rename to `data/videos/session_{session_id}_{camera_id}.{ext}`. `/chirp.wav` returns the reference sync chirp. `State.events()` collapses each session into one summary row (cameras present, status, received-at mtime, ball-frame counts, triangulation stats) for the `/events` JSON endpoint and the `/` HTML dashboard. A pitch arriving during an armed session auto-ends the session (one-shot arm) and flips commands to `disarm` for the echo window.
...
- `viewer.py` — two HTML renderers. `render_scene_html(scene)` produces the Plotly 3D figure (ground plane, world axes, camera markers + forward arrows, ray bundle per camera, triangulated trajectory coloured by time; loads Plotly.js from CDN). `render_events_index_html(events, devices, session)` produces the `/` dashboard: devices panel (A/B online badges), session panel (ARMED/IDLE + id), Arm/Cancel HTML-form buttons, and the events table with per-row `single`/`dual` mode chip + links into `/viewer/{session_id}`. A 1 Hz vanilla-JS tick re-polls `/status` to keep the top panels live without reloading the events table. No JS framework, no build step.
