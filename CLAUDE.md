...
Time-sync is a separate orthogonal flow: the operator taps 時間校正 → `.timeSyncWaiting` → (chirp detected) → `.standby`. Not gated by arm.
...
- **`payload`** (`application/json`) — encoded `ServerUploader.PitchPayload` ↔ `main.PitchPayload`:
  ...
  session_id: str matching ^s_[0-9a-f]{4,32}$     # server-minted; sole pairing key
  ...
  video_fps: float                                # nominal capture rate (240.0 in the default rig)
  ...
  **No `frames` field on the wire.** Server detection populates it post-ingest.
...
`CameraViewController` runs a 1 Hz `POST /heartbeat` loop (base cadence from Settings → Heartbeat Interval, default 1 s). On failure the interval doubles until a 60 s cap; a success resets to the base. A manual **Test** button on the HUD cancels any in-flight probe and re-probes immediately. The HUD also shows `Last contact: N s ago` updated by a 1 Hz tick timer (paused with the view). A generation token on each probe ensures stale responses from an outdated host/port can't overwrite current state. The HUD's `Server` line shows `ARMED (s_xxx)` / `IDLE · last: cancelled` derived from the heartbeat's `session` payload.
