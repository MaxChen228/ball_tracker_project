...
- `pipeline.py` — glue. `detect_pitch(mov_path, video_start_pts_s, hsv_range?, frame_iter?, *, shape_gate?, selector_tuning?, chain_filter_params?, expected_radius_px?, should_cancel?)` decodes the MOV and runs the same byte-aligned HSV → CC → shape gate → temporal selector pipeline as iOS-live (no MOG2, no morphology). Synthesises one `FramePayload` per decoded sample on the iOS session clock; runs `chain_filter.annotate` once at the end. Tests inject a stub iterator to avoid real decoding.
