...
def _no_triangulate(_cam_lo, _cam_hi, _a, _b):
    return []
...
def test_dedupe_key_extended_with_candidate_idx():
    ...
    def _two_points(_cam_lo, _cam_hi, _a, _b):
...
def test_dedupe_key_canonicalized_across_cam_directions():
    ...
    sess1.ingest("A", _frame(7, 0.0, candidates=cands),
                 lambda _cl, _ch, _a, _b: [_make_point(0.0, 0, 0)])
    ...
    # Same canonical key under both ingest directions: cam pair (A,B),
    # A frame_idx 7, B frame_idx 5, both candidate indices 0. The cam-pair
    # prefix (Phase 2a N-cam dedup) is direction-invariant too.
    assert keys1 == {("A", "B", 7, 5, 0, 0)}
    assert keys2 == {("A", "B", 7, 5, 0, 0)}
...
def test_fan_out_emits_all_passing_pairs():
    ...
    def _six_points(_cam_lo, _cam_hi, _a, _b):
        return [
...
def test_fan_out_skips_within_pair_dedupe():
    ...
    def _duplicate_keys(_cam_lo, _cam_hi, _a, _b):
