// === submit + preview click handlers ===
  ...
  document.addEventListener('submit', async (e) => {
    const form = e.target;
    const m = form.action && form.action.match(/\/sessions\/([^/]+)\/(trash|restore|delete|cancel_processing|resume_processing)$/);
    if (m) {
      e.preventDefault();
      const sid = m[1];
      const op = m[2];
      ...
      // disappear within ~1 frame instead of waiting for the polling tick.
      // tickEvents() at the end reconciles real state, so a fetch reject
      // naturally rolls the optimistic mutation back.
      if (op === 'cancel_processing') {
        ...
        if (target) {
          ...
          // back in 5 s later; 'canceled' keeps the chip in place.
          target.processing_state = 'canceled';
          _lastEvKey = null;
