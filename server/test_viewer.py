...
# ---- Ballistic fit overlay ------------------------------------------------


def test_build_scene_attaches_ballistic_curves_from_session_result():
    """When `session_result.ballistic_by_path` carries a summary with
    `params` populated, `build_scene` should sample the fitted curve and
    expose it on `scene.ballistic_curves[path]` with `n_samples=100`
    tuples spanning the inlier time range."""
    from ballistic_fit import fit_ballistic_ransac
    from schemas import BallisticSummary, SessionResult, TriangulatedPoint

    # Synthetic parabola: z(t) = 1.2 + 4·t - 0.5·9.81·t²
    ...
    pts = [
        TriangulatedPoint(
            ...
            x_m=float(0.1 * t),
            y_m=float(0.2 + 3.0 * t),
            z_m=float(1.2 + 4.0 * t - 0.5 * 9.81 * t * t),
            residual_m=1e-6,
        )
        for t in ts
    ]
    fit = fit_ballistic_ransac(pts)
    assert fit is not None, "setup: RANSAC should succeed on clean parabola"
    ...
    result = SessionResult(
        ...
        points=pts,
        triangulated=pts,
        triangulated_by_path={"server_post": pts},
        ballistic_by_path={"server_post": summary},
    )

    K, (R_a, t_a, _, H_a), (R_b, t_b, _, H_b) = _make_rig()
    pa = _pitch("A", 42, K, R_a, t_a, H_a, np.array([[0.1, 0.3, 1.0]]))
    pb = _pitch("B", 42, K, R_b, t_b, H_b, np.array([[0.1, 0.3, 1.0]]))
    ...
    assert "server_post" in scene.ballistic_curves
    curve = scene.ballistic_curves["server_post"]
    assert len(curve) == 100
    # Every sample is a 4-tuple (t, x, y, z).
    assert all(len(pt) == 4 for pt in curve)
    # Curve t-range should straddle the inlier time range.
    t_samples = [pt[0] for pt in curve]
    inlier_ts = [pts[i].t_rel_s for i in summary.inlier_indices]
    assert t_samples[0] == pytest.approx(min(inlier_ts))
    assert t_samples[-1] == pytest.approx(max(inlier_ts))
    # Curve should round-trip through to_dict → JSON cleanly.
    out = _json.loads(_json.dumps(scene.to_dict()))
    assert "ballistic_curves" in out
    assert "server_post" in out["ballistic_curves"]
    assert len(out["ballistic_curves"]["server_post"]) == 100


def test_build_scene_skips_ballistic_when_no_session_result():
    """No session_result → empty ballistic_curves (no silent fallback)."""
    K, (R_a, t_a, _, H_a), _ = _make_rig()
    pitch = _pitch("A", 1, K, R_a, t_a, H_a, np.array([[0.1, 0.3, 1.0]]))
    scene = build_scene(sid(1), {"A": pitch}, triangulated=None)
    assert scene.ballistic_curves == {}


def test_build_scene_skips_ballistic_when_inliers_below_threshold():
    """N<7 inliers recorded on the summary → path skipped, no curve."""
    from schemas import BallisticSummary, SessionResult, TriangulatedPoint

    pts = [
        TriangulatedPoint(t_rel_s=i * 0.01, x_m=0.0, y_m=0.0, z_m=1.0, residual_m=0.0)
        for i in range(10)
    ]
    summary = BallisticSummary(
        release_point_m=(0.0, 0.0, 1.0),
        release_velocity_mps=(0.0, 0.0, 0.0),
        speed_mps=0.0,
        speed_mph=0.0,
        g_fit=9.81,
        g_mode="free",
    ...
    result = SessionResult(
        ...
        camera_b_received=True,
        points=pts,
        triangulated_by_path={"live": pts},
        ballistic_by_path={"live": summary},
    )
    K, (R_a, t_a, _, H_a), _ = _make_rig()
    pitch = _pitch("A", 43, K, R_a, t_a, H_a, np.array([[0.1, 0.3, 1.0]]))
    scene = build_scene(
        sid(43),
        {"A": pitch},
        triangulated=pts,
