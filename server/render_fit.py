...
Static layers (ground / plate / strike zone / world axes / cameras) come
from `render_scene_static.static_traces`; layout chrome comes from
`render_scene_layout`. This module only owns the segment-specific
visuals (per-segment palette, fit curve, seed velocity arrow, background
points) plus the page wrapper HTML.
"""
...
def build_fit_figure(
    ...
    # Fit page keeps its own slightly different default camera (closer
    # eye, lower centre) — better for inspecting low-altitude segments.
    # Phase 1 preserves this; phase 5 sweep can decide whether to unify.
    fit_camera = dict(
    ...
    fig.update_layout(**default_layout_kwargs(
        scene=default_scene_block(camera=fit_camera),
    ))
    return fig
