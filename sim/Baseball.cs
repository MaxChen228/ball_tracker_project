...
public partial class Baseball : Node3D
{
	...
	public override void _Ready()
	{
		...
		pitchDecisionMenu = GetNodeOrNull<PitchDecisionMenu>("/root/UI/PitchDecisionMenu");
		if (pitchDecisionMenu != null)
		{
			pitchDecisionMenu.DecisionSelected += OnManualDecisionReceived;
			GD.Print("✅ [Baseball.cs] Found PitchDecisionMenu at /root/UI/PitchDecisionMenu");
		}
		else
		{
			// Fallback: try ../UI/PitchDecisionMenu
			pitchDecisionMenu = GetNodeOrNull<PitchDecisionMenu>("../UI/PitchDecisionMenu");
			if (pitchDecisionMenu != null)
			{
				pitchDecisionMenu.DecisionSelected += OnManualDecisionReceived;
				GD.Print("✅ [Baseball.cs] Found PitchDecisionMenu at ../UI/PitchDecisionMenu");
			}
			else
			{
				GD.PrintErr("❌ [Baseball.cs] PitchDecisionMenu NOT FOUND! Tried /root/UI/ and ../UI/");
			}
		}
	...
	private void PredictTrajectory()
	{
		...
		while (simVel.LengthSquared() > 0.001f && step < maxSteps)
		{
			...
			if (dist >= wallDistance && simPos.Y <= wallHeight)
			{
				// 計算碰撞點：把球投影到牆壁上（精確碰撞位置）
				Vector2 normal2D = new Vector2(simPos.X, simPos.Z).Normalized();
				Vector3 normal = new Vector3(normal2D.X, 0f, normal2D.Y);
				
				// 碰撞位置就在牆壁表面
				simPos = new Vector3(normal.X * wallDistance, simPos.Y, normal.Y * wallDistance);
				
				// 反射速度向量：v' = v - 2(v·n)n
				float dot = simVel.X * normal.X + simVel.Z * normal.Y;
				simVel.X -= 2f * dot * normal.X;
				simVel.Z -= 2f * dot * normal.Y;
				// 反彈能量損耗
				simVel *= 0.6f;
			}
	...
	private void ResetPitch(string jsonString)
	{
		...
		try
		{
			...
			{
				GD.PrintErr($"❌ [Baseball.cs] Deserialized currentHitData is null!");
				return;
			}
			GD.Print($"✅ [Baseball.cs] Deserialized OK: EV={currentHitData.ExitVelocityMph}, LA={currentHitData.LaunchAngleDeg}, SA={currentHitData.SprayAngleDeg}, SR={currentHitData.SpinRateRpm}, PX={currentHitData.PitchXM}, PY={currentHitData.PitchYM}, Call={currentHitData.PitchCall}");
			...
			if (pitchDecisionMenu != null)
			{
				GD.Print("✅ [Baseball.cs] Showing PitchDecisionMenu");
				// 使用 Show() 而非直接設 visible，確保節點正確顯示
				pitchDecisionMenu.Show();
				// 同時切換滑鼠模式
				Input.MouseMode = Input.MouseModeEnum.Visible;
			}
			else
			{
				GD.PrintErr("❌ [Baseball.cs] pitchDecisionMenu is NULL! Cannot show menu.");
			}
		}
	...
	// --- 核心邏輯：接收手動輸入數據 ---
	public void ManualPitchData(string json)
	{
		GD.Print($"[Baseball.cs] ManualPitchData called with JSON: {json}");
		ResetPitch(json);
	}
	...
	{
		float x = GlobalPosition.X, y = GlobalPosition.Y, z = GlobalPosition.Z;
		float dist = Mathf.Sqrt(x * x + z * z);
		bool isFoul = (z > 0) || (Mathf.Abs(x) > Mathf.Abs(z));

		// 撞牆反彈（取代原本的停球）
		if (!isFoul && dist >= wallDistance && !hasPassedWall)
		{
			if (y <= wallHeight)
			{
				// 計算碰撞點：把球投影到牆壁上（精確碰撞位置）
				Vector2 normal2D = new Vector2(x, z).Normalized();
				Vector3 normal = new Vector3(normal2D.X, 0f, normal2D.Y);
				
				// 碰撞位置就在牆壁表面
				GlobalPosition = new Vector3(normal.X * wallDistance, y, normal.Z * wallDistance);
				...
				// 反彈能量損耗
				velocity *= 0.6f;
				GD.Print($"[Baseball] 撞牆反彈！速度剩餘：{velocity.Length():F1} m/s");
			}
			else if (!hasBounced)
			{
				hasPassedWall = true;
				displays?.ShowHitResult($"HOME RUN!!\n{dist:F1} m");
				isSimulating = false;
			}
			else
			{
				isSimulating = false;
				displays?.ShowHitResult($"GR DOUBLE\n{dist:F1} m");
			}
		}

		if (y <= 0.037f && !isRolling)
		{
			GlobalPosition = new Vector3(x, 0.037f, z);
			hasBounced = true;
