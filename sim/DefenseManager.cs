...
public partial class DefenseManager : Node3D
{
    ...
    {
        if (ActiveFielder == null) return;

        // 當守備員還在跑動時，動態更新他的目標點（跟隨球的預測落點）
        if (ActiveFielder.CurrentState == Fielder.FielderState.Running)
        {
            // 用球的 Y=0 投影點作為新的目標
            Vector3 newTarget = new Vector3(realBallPos.X, 0, realBallPos.Z);
            // 只有當新目標比舊目標更接近球的當前位置時才更新
            float distToBall = (newTarget - realBallPos).Length();
            float oldDistToBall = (ActiveFielder.TargetPosition - realBallPos).Length();
            if (distToBall < oldDistToBall)
            {
                ActiveFielder.TargetPosition = newTarget;
            }
        }
        // 當守備員已到達預測點開始「等球」時，啟動微調追蹤球的實際位置
        else if (ActiveFielder.CurrentState == Fielder.FielderState.Catching)
        {
