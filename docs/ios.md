...
`dispatchDetectionIfDue` runs on every sample — throttled to 60 Hz on a utility queue so the ~12-18 ms HSV + CC + shape pipeline (1080p full-frame, CPU) can't stall 240 fps capture. The throttle is the current ceiling: to push detection toward 240 Hz, shrink per-frame cost first (720p capture, Metal/vImage HSV conversion). ROI tracking around the last hit is **explicitly out** — the previous design used `BTStatefulBallDetector` with a ±3× radius ROI but a static distractor matching the ball HSV (e.g. a plant leaf) could lock the ROI for seconds, making live diverge silently from server_post. Stateless full-frame on every frame keeps live ↔ server_post byte-aligned end-to-end. `CameraRecordingWorkflow` is kicked off on the first captured sample using the sample's session-clock PTS as `videoStartPtsS`, regardless of which paths are active.
