...
`dispatchDetection` runs `detectionPool.enqueue` on every captured sample — there is no producer-side throttle. The pool's serial worker chews ~15 ms/frame of full-frame HSV+CC+shape (1080p, CPU), so on a single P-core effective throughput is ~66 detections/s; the remaining frames drop at `enqueue` when the in-flight count hits `maxBacklog` (8). `droppedFrameCount` is surfaced on the dashboard so the operator can see when detection is falling behind capture. ROI tracking around the last hit is **explicitly out** — the previous design used `BTStatefulBallDetector` with a ±3× radius ROI cutting cost to ~3 ms/hit, but a static distractor matching the ball HSV (e.g. a plant leaf) could lock the ROI for seconds, making live diverge silently from server_post. 66 Hz of full-frame distractor-immune detection is more useful for ballistic fit than 240 Hz that misses the ball; if we ever need to lift the ceiling the right path is shrinking per-frame cost (720p capture, Metal/vImage HSV) rather than re-introducing ROI. `CameraRecordingWorkflow` is kicked off on the first captured sample using the sample's session-clock PTS as `videoStartPtsS`, regardless of which paths are active.
