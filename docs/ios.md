...
- `.recording`: session spun up at `trackingFps` (240). Captures H.264 MOV via `ClipRecorder`. **Only exit path is `forceFinishIfRecording()`**, triggered when the dashboard sends `disarm` (operator pressed Stop) or the server-side session times out. No on-device detection, no auto-end. The `PitchPayload` lands in `PitchPayloadStore` and the workflow fires the `onCycleCompleted: () -> Void` callback to advance the state machine; the clip finishes async before the payload is persisted. Session is stopped on the way back to standby. Persist + enqueue runs synchronously before the state transition flips back to `.standby`.
...
`dispatchDetection` runs `detectionPool.enqueue` on every captured sample — there is no producer-side throttle. The pool's serial worker chews ~15 ms/frame of full-frame HSV+CC+shape (1080p, CPU), so on a single P-core effective throughput is ~66 detections/s; the remaining frames drop at `enqueue` when the in-flight count hits `maxBacklog` (8). `droppedFrameCount` is surfaced on the dashboard so the operator can see when detection is falling behind capture. ROI tracking around the last hit is **explicitly out** — full-frame distractor-immune detection beats faster ROI-locked detection that can stick on a static HSV match (e.g. a plant leaf) and silently diverge from server_post. If we ever need to lift the ceiling the right path is shrinking per-frame cost (720p capture, Metal/vImage HSV) rather than re-introducing ROI. `CameraRecordingWorkflow` is kicked off on the first captured sample using the sample's session-clock PTS as `videoStartPtsS`, regardless of which paths are active.
...
- `AppSettingsStore.swift` — the only persisted iOS settings store. Reads `server_ip` + `camera_role` from `UserDefaults` at launch. `purgeLegacyKeys()` strips pre-Phase-1 keys (intrinsics, HSV, capture mode, chirp threshold, etc.) once per launch — those are now server-owned and arrive over WS. **There is no in-app settings UI**: operator edits these via the Xcode build config / first-launch defaults, and everything else is pushed by the server.
- **ChArUco intrinsics is the sole in-app functional flow.** Tap **Calibrate** in the overlay → `IntrinsicsCalibrationViewController` presents a modal with its own `AVCaptureSession` (`CameraCaptureRuntime.pauseSession()` hands off the back camera). `IntrinsicsCaptureController` runs the auto-trigger state machine: ChArUco preview eval at 10 Hz + focus-stable gate + sharpness rolling median + pose-fingerprint dedup, snaps 4032×3024 JPEGs via `AVCapturePhotoOutput` until pose diversity (yaw/pitch/distance bins) is covered. `CharucoCalibrator.mm` is the Obj-C++ OpenCV bridge; same 5×7 / 0.040 / 0.030 / DICT_4X4_50 board spec as the deleted `server/calibrate_intrinsics.py` CLI. Solve result writes to `Documents/intrinsics-pending.json` first, then POST `/calibration/intrinsics/{device_id}` ([routes/calibration_intrinsics.py:65](../server/routes/calibration_intrinsics.py:65)); pending file is cleared only on 200. Upload failure leaves pending in place — re-entering the VC offers "retry upload" instead of forcing a re-shoot. Successful upload triggers a delayed `POST /calibration/auto/{cam}` so the dashboard auto-cal picks up the fresh ChArUco K without operator intervention. The runtime's `intrinsicsCalibrationActive` flag gates the same WS commands as `calCaptureState != .idle` for the duration.
